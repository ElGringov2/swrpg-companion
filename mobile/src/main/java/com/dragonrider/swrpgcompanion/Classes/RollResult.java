...
public class RollResult implements Cloneable {
    ...
    public void PopulateFullView(ViewGroup diceGroup) {
		...
		for (int i = 0; i <= 6; i++)
		{
			int imageID = diceGroup.getContext().getResources().getIdentifier(String.format("dice_%d", i), "drawable", diceGroup.getContext().getPackageName());
			for (int iDice = 0; iDice < values1[i]; iDice++) {
				ImageView iv = new ImageView(diceGroup.getContext());
				iv.setLayoutParams(params);
				iv.setImageResource(imageID);
				iv.setAdjustViewBounds(true);
				diceGroup.addView(iv);
				
				iv = new ImageView(diceGroup.getContext());
				iv.setLayoutParams(params);
				iv.setAdjustViewBounds(true);
				iv.setImageResource(getResultImageResource(diceGroup.getContext(), DiceResult(iPosition)));
				diceGroup.addView(iv);
				iPosition++;
    ...
    public void PopulateLightView(ViewGroup diceGroup) {
